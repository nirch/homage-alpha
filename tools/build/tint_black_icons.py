...
ICONS_SET_SRC_PATH = "resources/black icons"
ICONS_SET_DEST_PATH = "output/tinted icons"
...
def tint_icons(src, dest):
    ...
    for icon in icons:
        ...
        # tint the icon
        tint = 12/360.0
        img = Image.open(os.path.join(ICONS_SET_DEST_PATH, icon)).convert('RGBA')
        arr = np.array(img)
        new_img = Image.fromarray(shift_hue(arr, 450), 'RGBA')
        new_img.save(os.path.join(ICONS_SET_DEST_PATH, icon))

# The script
if __name__ == '__main__':
    tint_icons(src=ICONS_SET_SRC_PATH, dest=ICONS_SET_DEST_PATH)
